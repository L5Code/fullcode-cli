...
const createCommands = () => {
	...
	program
		...
		.action(async (name) => {
			await addKoaRouterAction(name, program._optionValues.dest || 'routes');
		});
	...
	program
		...
		.action(async (name) => {
			await addVueCpnAction(name, program._optionValues.dest || 'src/components');
		})
	...
	program
		...
		.action(async (name) => {
			await addVuePageAndRouter(name, program._optionValues.dest || `src/pages/${name.toLowerCase()}`);
		})
	...
	program
		...
		.action(async (name) => {
			await addStoreAction(name, program._optionValues.dest || `src/store/modules/${name.toLowerCase()}`);
		})
};
